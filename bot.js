...
function logMessage(msg) {
  ...
  log.push({ messageId: msg.message_id, userId: msg.from.id, username: msg.from.username ? msg.from.username.toLowerCase() : null });
...
bot.onText(cmdExact("id"), async function(msg) {
  ...
  if (!target) return;
  await bot.sendMessage(msg.chat.id, "🪪 *ID de usuario*\n" + DIVIDER + "\n👤 " + target.first_name + "\n🔢 `" + target.id + "`", { parse_mode: "Markdown" });
});
...
bot.onText(/^[!/]purge(?:@\S+)?(?:\s+(\d+))?(?:\s+@(\S+))?$/i, async function(msg, match) {
  ...
  let deleted = 0;
  const targets = targetUsername ? log.filter(function(m) { return m.username === targetUsername; }).slice(-count) : log.slice(-count);
  ...
  const confirm = await bot.sendMessage(chatId, "🗑 *" + deleted + " mensajes eliminados" + (targetUsername ? " de @" + targetUsername : "") + ".*", { parse_mode: "Markdown" });
...
bot.onText(cmd("lock"), async function(msg, match) {
  ...
  let durationMs = null;
  if (durationStr) { durationMs = parseDuration(durationStr); if (!durationMs) { await sendTemp(chatId, "❌ Formato inválido. Usa: 30s, 5m, 1h, 2d"); return; } }
  try {
    ...
    await bot.sendMessage(chatId, "🔒 *Chat bloqueado" + (durationMs ? " por *" + durationStr + "*" : "") + "*\n" + DIVIDER + "\nSolo los administradores pueden escribir.", { parse_mode: "Markdown" });
    if (durationMs) {
...
bot.on("message", async function(msg) {
  ...
  try {
    ...
    await bot.sendMessage(chatId, "📢 *Mención masiva* — " + count + " miembros\n" + DIVIDER + "\n👤 Por: " + (msg.from && msg.from.first_name) + "\n⚠️ Todos han sido notificados.", { parse_mode: "Markdown", reply_to_message_id: msg.message_id });
...
bot.on("message", async function(msg) {
  ...
  try {
    ...
    await bot.sendMessage(chatId, "🗑 *Cuenta eliminada removida*\n" + DIVIDER + "\n🔢 ID: `" + msg.from.id + "`\n✅ Removido automáticamente.", { parse_mode: "Markdown" });
  } catch {}
});
...
bot.on("message", async function(msg) {
  ...
  if (msg.new_chat_title) { await bot.sendMessage(chatId, "✏️ *Nombre del grupo cambiado*\n" + DIVIDER + "\n📛 Nuevo nombre: *" + msg.new_chat_title + "*\n👤 Por: " + ((msg.from && msg.from.first_name) || "Desconocido"), { parse_mode: "Markdown" }); }
  if (msg.new_chat_photo) { await bot.sendMessage(chatId, "🖼 *Foto del grupo cambiada*\n" + DIVIDER + "\n👤 Por: " + ((msg.from && msg.from.first_name) || "Desconocido"), { parse_mode: "Markdown" }); }
  if (msg.delete_chat_photo) { await bot.sendMessage(chatId, "🗑 *Foto del grupo eliminada*\n" + DIVIDER + "\n👤 Por: " + ((msg.from && msg.from.first_name) || "Desconocido"), { parse_mode: "Markdown" }); }
  if (msg.pinned_message) { await bot.sendMessage(chatId, "📌 *Mensaje anclado*\n" + DIVIDER + "\n👤 Por: " + ((msg.from && msg.from.first_name) || "Desconocido"), { parse_mode: "Markdown" }); }
});
...
bot.onText(cmdExact("iniciar"), async function(msg) {
  await bot.sendMessage(msg.chat.id,
    ...
    { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "📋 Help", callback_data: "help" }], [{ text: "🏓 Ping", callback_data: "ping" }, { text: "🏠 Server Info", callback_data: "serverinfo" }]] } });
});
...
bot.on("callback_query", async function(query) {
  ...
  if (query.data === "ping") {
    ...
    const latency = Date.now() - start;
    await bot.editMessageText("🏓 *Pong!*\n" + DIVIDER + "\n⚡ Latencia: `" + latency + "ms`", { chat_id: chatId, message_id: sent.message_id, parse_mode: "Markdown" });
  }
  if (query.data === "serverinfo") {
    await bot.answerCallbackQuery(query.id);
    if (msg.chat.type !== "group" && msg.chat.type !== "supergroup") { await bot.sendMessage(chatId, "❌ Este comando solo funciona en grupos."); return; }
    try {
      const chat = await bot.getChat(chatId);
      const memberCount = await bot.getChatMemberCount(chatId);
      const admins = await bot.getChatAdministrators(chatId);
      await bot.sendMessage(chatId, "🏠 *Info del Grupo*\n" + DIVIDER + "\n📛 Nombre: " + (chat.title || "Sin nombre") + "\n🔢 ID: `" + chat.id + "`\n🔗 Username: " + (chat.username ? "@" + chat.username : "Sin username") + "\n👥 Miembros: " + memberCount + "\n🛡 Admins: " + admins.length + "\n📝 Tipo: " + chat.type + "\n" + DIVIDER, { parse_mode: "Markdown" });
